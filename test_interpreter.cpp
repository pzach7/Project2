...
TEST_CASE("Test graphic types", "[interpreter]") {
  ...
  {
    std::string program = "(point 10 0)";
    std::istringstream iss(program);
    Interpreter interp;

    REQUIRE(interp.parse(iss));
    REQUIRE(interp.eval() == Expression(std::make_tuple(10., 0.)));
  }
  {
...
TEST_CASE( "Test Interpreter doesn't clear environment", "[interpreter]" ) {
    Environment freshEnv = Environment();
    Interpreter interp = Interpreter();
    
    REQUIRE(freshEnv.getMapSize() == interp.getEnv().getMapSize());
}


TEST_CASE( "Test Interpreter set environment", "[interpreter]" ) {
    std::string program = "(define a 42)";
    Interpreter interp = Interpreter();
    Environment preRunEnv = interp.getEnv();
    ...
    Expression result;
    interp.eval();
    REQUIRE(preRunEnv.getMapSize() != interp.getEnv().getMapSize());
    
    interp.setEnv(preRunEnv);
    REQUIRE(preRunEnv.getMapSize() == interp.getEnv().getMapSize());
